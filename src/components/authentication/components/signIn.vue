...
const schema = yup.object({
  ...
  configPassword: yup
    .string()
    .oneOf([yup.ref("password")], "Please enter a valid password"),
});
...
const { value: configPasswordValue, errorMessage: configPasswordError } =
  useField("configPassword");
...
function postNewUserData() {
  const params: signUpRequest = {
    username: usernameValue.value as string,
    password: passwordValue.value as string,
    name: name.value as string,
    email: emailValue.value as string,
  };
  const store = userData();
  AuthenticationApi.postSignUpData(params).then((response) => {
    console.log(response);
    store.setUserName(response.username);
    store.setEmail(response.email);
    prosesLogin(response.username, params.password);
  });
}

function prosesLogin(username: string, password: string) {
  console.log("qwertyuiopasdfghjk");
  const params: loginRequest = {
    username: username,
    password: password,
  };
  AuthenticationApi.postLoginData(params).then((response) => {
...
<template>
  <div class="flex-col">
    <div
      class="flex flex-col justify-between items-center w-[500px] h-[500px] pb-6 pt-6"
    >
    ...
    <div class="flex mt-3 mb-9">
      <div class="text-white text-xs flex items-center mx-auto gap-1">
        if you have account ?
...
.back {
  ...
  border-radius: 2em;
  box-shadow:
    -5px 0 10px rgb(140, 12, 147),
    5px 0 10px rgb(85, 31, 200);
...
.buton1:hover {
  transform: translateY(10px);
  box-shadow:
    -10px 0 50px rgba(241, 0, 108, 0.352),
